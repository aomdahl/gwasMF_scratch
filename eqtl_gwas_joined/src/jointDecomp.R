################
# Ashton Omdahl, Nov 1, 2021
# Script/functions to aid in rapid joint analysis of 2 related data matrices
#
################
...
maxfValue <- function(dat) {
  ...
  data.frame(t(apply(dat[,2:50], 2, function(x) x[which.max(abs(x))]))) #get the max snp-gene pair, snp fixed.
}



#Options are "RANDOM", "MAX_AVG_Z", "CENTER_Z", "MAX_GENE"
#parseEQTLs(args$eqtl_source, args$eqtl_selection)
parseEQTLs <- function(dir, sel.criteria)
{
...
#Rscript jointDecomp.R --eqtl_source
parser <- ArgumentParser$new()
parser$add_description("Script to prep the projected loadings for LDSC")
parser$add_argument("--eqtl_source", type = 'character', help = "Source dir for eQTLs")
parser$add_argument("--gwas_source", type = 'character', help = "Z score file for GWAS")
parser$add_argument("--gwas_names", type = 'character', 
                    help = "File containing the names of the traits/gwas studies", 
                    default = "/work-zfs/abattle4/ashton/snp_networks/gwas_decomp_ldsc/trait_selections/seed2_thresh0.9_h2-0.1.names.tsv")
parser$add_argument("--eqtl_names", type = 'character', help = "File containing the names of eqtl studies")
parser$add_argument("--debug", type = "logical", help = "Specify to run in debug mode", default = FALSE, action = "store_true")
parser$add_argument("--output", type = "character", help = "Path to output location.")
parser$add_argument("--na_method", type = "character", help = "Method to account for NAs in data; options are to zero out (", 
                    default = "ZERO")
parser$add_argument("--eqtl_selection", type = "character", help = "Method to select eqlts (optsion are Random, mean max, median)", 
                    default = "RANDOM")
parser$add_argument("--covar_control", type = "logical", action = "store_true", default = FALSE, 
                    help = "Specify this flag if you want to control for covariates. Currently just using the default reference files, see code.")
parser$add_argument('--help',type='logical',action='store_true',help='Print the help page')
parser$helpme()
args <- parser$get_args()
#Bed conversion:
#BED <- "/work-zfs/abattle4/ashton/reference_data/1000G_UKBB_intersect.no_ambig.no_multi.hg38.bed"
BED <- "/work-zfs/abattle4/ashton/reference_data/1000G_UKBB_intersect.no_ambig.no_multi.hg38.5col.bed"
if(FALSE & args$debug)
{
  
  #Rscript jointDecomp.R --gwas_names gwas_extracts/trait.names.txt --eqtl_names eqtl_extracts/tiss.names.txt --output ./results/gwas_centric_1e-5/500kb_0.1r2/covar_ --na_method 'ZERO' --covar_control
  #random run
  setwd("/work-zfs/abattle4/ashton/snp_networks/scratch/eqtl_gwas_joined/fourth_pass_universal/")
  args <- list()
  args$gwas_names <-"gwas_extracts/trait.names.txt"

  args$eqtl_source = "./eqtl_extracts/500kb_0.1r2/"
  args$gwas_source = "./gwas_extracts/500kb_0.1r2/rand.z.tsv"
  args$eqtl_names = "eqtl_extracts/tiss.names.txt" 
  args$na_method = "ZERO"
  args$eqtl_selection = "RANDOM"
  args$covar_control <- FALSE
  args$debug <- FALSE
  args$output <- "./results/500kb_0.1r2/"
  args$eqtl_source = "./eqtl_extracts/500kb_0.1r2/"
  args$gwas_source = "./gwas_extracts/500kb_0.1r2/rand.z.tsv"
  
  #Sqtl run
  args$eqtl_source = "./sqtl_extracts/gwas_centric_5e-8/500kb_0.1r2/"
  args$output <- "./results/sqtl_gwas_centric_5e-8"
  
}

#Parse in study name information
trait.names <- fread(args$gwas_names, header = FALSE)
eqtl.names <-scan(args$eqtl_names, what = character())
ADJ <- args$covar_control
all.study.counts <- getStudyCounts(args$covar_control, trait.names, eqtl.names)


#Read in eQTLS
message("Loading eQTLs into memory...")
print(args$eqtl_selection)
eqtls.z <- parseEQTLs(args$eqtl_source, args$eqtl_selection)
message(paste0(dim(eqtls.z)[1], " SNPS have eQTL summary stats for analysis, with filtering done."))

#GWAS parsing
message("loading GWAS into memory...")
gwas.z <- fread(args$gwas_source) %>% rename("hg19" = ids)

if(length(unique(gwas.z$hg19)) !=length((gwas.z$hg19)) )
{
  message("GWAS has multi-allelic SNPs included, consider reviewing input list...")
  quit(save = "no")
  
}
gwas.names <- alignGWASNames( trait.names, colnames(gwas.z))
message(paste0(dim(gwas.z)[1], " SNPS have GWAS summary stats for analysis"))
message(paste0(".... across ", dim(gwas.z)[2], " studies"))

if(args$debug)
{
  #sanity check on eqtl names
  files <- paste0(args$eqtl_source, list.files(args$eqtl_source, pattern = "*.extract.tsv"))
  x <- eqtl.names[10] != (gsub(pattern = ".*/rand_", 
       replacement = "", files) %>% 
    gsub(pattern = ".allpairs.extract.tsv",replacement = "", x = .))[10]
    x_other <- !((gwas.names[1] == "Pulse_rate,_automated_reading") && (colnames(gwas.z)[2] == "102_irnt"))
    if ( x | x_other) {
    message("eQTL or gwas names given don't line up. Issue")
    #print(eqtl.names)
    #print(gsub(pattern = ".*/rand_", 
    #           replacement = "", files) %>% 
    #        gsub(pattern = ".allpairs.extract.tsv",replacement = "", x = .))
    print(colnames(gwas.z))
    print(gwas.names )
    readline()
    quit(save = "no")
  }
...
all.names <- c(eqtl.names,gwas.names) #order matters: eqlts come first.
#all.names <- c(eqtl.names, gwas.names) #ADJ

ngwas <- length(gwas.names)
neqtl <- length(eqtl.names)

if(ADJ) #set order of count list
{
  all.study.counts$study <- factor(all.study.counts$study, levels = all.names)
  all.study.counts <- all.study.counts %>% arrange(study) #order looks good.
}

#BED conversion from hg38 to 19
ukbb_1kg.bed <- fread(BED) %>%
  magrittr::set_colnames(c("chr", "start", "stop", "hg19", "hg19_ukbb")) %>% filter(hg19 %in% gwas.z$hg19)

#Map hg19
#change: added in full names
gwas.z <-  left_join((ukbb_1kg.bed %>% mutate("hg38" =gsub(x = paste0(chr,":", stop), pattern = "chr", replacement = "" )) %>% 
                        select(hg38, hg19, hg19_ukbb)), gwas.z, by = "hg19") %>% arrange(hg38) %>% 
  separate(hg19_ukbb,sep = ":", into = c("f", "s", "ref", "alt")) %>% select(-f, -s) %>%
  mutate("variant_id" = paste0(hg38, ":", ref, ":", alt)) %>% magrittr::set_colnames(c("hg38","hg19","ref","alt",gwas.names, "variant_id"))

#join the data #
joint.data <- left_join(eqtls.z, gwas.z, by = "variant_id") %>% arrange(hg38) 
joint.meta.data <- joint.data %>% select(hg38, hg19, variant_id, ref, alt, gene_id)
joint.data <- list("mat" = as.matrix(joint.data %>% select(-hg38, -hg19, -variant_id,-ref,-alt, -gene_id)) %>% 
                     handleNAs(., args$na_method), "meta" = joint.meta.data)
print("Current data dim....")
print(dim(joint.data$mat))
#Only runs if have specified to residualize data.
joint.data$mat <- residualizeData(args$covar_control, joint.data$meta, joint.data$mat, all.study.counts, all.names, args)

message(paste0("joined data has ", dim(joint.data$mat)[1], " SNPs, ", dim(joint.data$mat)[2], " studies."))

#Perform the analysis

reg <- fullFactorizationAnalysis(joint.data$mat,all.names, ngwas, neqtl, "signed")
abs <- fullFactorizationAnalysis(abs(joint.data$mat),all.names, ngwas, neqtl, "unsigned")
#we want to remove the first few
pll <- append(reg$plots, reg$plots)
savePlots(c(reg$fnames, abs$fnames), pll, args$output)
