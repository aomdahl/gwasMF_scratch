...
echo "#SBATCH --time=120:00" >> bash_default.tmp
...
ls /data/abattle4/lab_data/GWAS_summary_statistics/PanUKBB/phenotype_flat_files/extract_EUR_scripts/*.sh | head -n 35 | sed 's/^/bash /g' >> slurm_first_quarter.sh

cp bash_default.tmp slurm_second_quarter.sh
sed -i 's/partial_extraction/second_extraction/g' slurm_second_quarter.sh
ls /data/abattle4/lab_data/GWAS_summary_statistics/PanUKBB/phenotype_flat_files/extract_EUR_scripts/*.sh | head -n 70 | tail -n 35 | sed 's/^/bash /g' >> slurm_second_quarter.sh

cp bash_default.tmp slurm_third_quarter.sh
sed -i 's/partial_extraction/third_extraction/g' slurm_third_quarter.sh
ls /data/abattle4/lab_data/GWAS_summary_statistics/PanUKBB/phenotype_flat_files/extract_EUR_scripts/*.sh | head -n 105 | tail -n 35 | sed 's/^/bash /g' >> slurm_third_quarter.sh

cp bash_default.tmp slurm_fourth_quarter.sh
sed -i 's/partial_extraction/fourth_extraction/g' slurm_fourth_quarter.sh
ls /data/abattle4/lab_data/GWAS_summary_statistics/PanUKBB/phenotype_flat_files/extract_EUR_scripts/*.sh | tail -n 32  | sed 's/^/bash /g' >> slurm_fourth_quarter.sh
