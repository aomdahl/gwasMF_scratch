...
n.snps <- 1000
snp.weights <- list()
snp.weights[[1]] <- c(rep(0,200), rep(0,800))
snp.weights[[2]] <- c(rep(0,200), rep(0,25),rep(0,775))
...
for(rep in 1:n.iter)
{
  ...
  y1 <- phenotypeBuilder(NULL, pop1.geno[,1:n.snps], 1, type = "1+2=3")
  y2 <- phenotypeBuilder(NULL, pop2.geno[,1:n.snps], 1, type = "1+2=3")
  y3 <- phenotypeBuilder(NULL, pop3.geno[,1:n.snps], 1, type = "1+2=3")
   ...
   second <- relatedOverlapBETTER(n.snps, i, 3, y2, N.tot = N_tot, 
    ...
    }else
    {
      write.table(data.frame(round(cbind(first$se, second$se, third$se),digits = 6)) %>% set_colnames(c("A1", "B1", "C1","A2", "B2", "C2","A3", "B3", "C3")),  
                  file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/null_matrix_spearman/complete_summary_stats/seed", seedi, ".replicate",rep,".", i, ".SE.csv"),
                  quote = FALSE,sep = ",", row.names = FALSE )
      write.table(data.frame(round(cbind(first$beta, second$beta, third$beta),digits = 6)) %>% set_colnames(c("A1", "B1", "C1","A2", "B2", "C2","A3", "B3", "C3")),  
                  file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/null_matrix_spearman/complete_summary_stats/seed", seedi, ".replicate",rep,".", i, ".BETA.csv"),
                  quote = FALSE,sep = ",", row.names = FALSE )
    }
