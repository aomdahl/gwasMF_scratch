   ...
   second <- relatedOverlapBETTER(n.snps, i, 3, y2, N.tot = N_tot, 
    ...
    }else
    {
      write.table(data.frame(round(cbind(first$se, second$se, third$se),digits = 6)) %>% set_colnames(c("A1", "B1", "C1","A2", "B2", "C2","A3", "B3", "C3")),  
                  file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/non_null_matrix/simulations_lower_herit_sumstats/seed", seedi, ".replicate",rep,".", i, ".SE.csv"),
                  quote = FALSE,sep = ",", row.names = FALSE )
      write.table(data.frame(round(cbind(first$beta, second$beta, third$beta),digits = 6)) %>% set_colnames(c("A1", "B1", "C1","A2", "B2", "C2","A3", "B3", "C3")),  
                  file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/non_null_matrix/simulations_lower_herit_sumstats/seed", seedi, ".replicate",rep,".", i, ".BETA.csv"),
                  quote = FALSE,sep = ",", row.names = FALSE )
    }
