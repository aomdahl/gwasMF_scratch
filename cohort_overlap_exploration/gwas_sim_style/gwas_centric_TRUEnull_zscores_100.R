...
write.table(thou.g.mafs.use, file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/null_matrix_spearman/true_null_z/seed", seedi, "MAF.csv"), quote =FALSE,sep = ",", row.names = FALSE )
...
for(rep in 1:n.iter)
{
  ...
  y1 <- phenotypeBuilder(NULL, pop1.geno, 1, type = "0,0,0")
  y2 <- phenotypeBuilder(NULL, pop2.geno, 1, type = "0,0,0")
  ...
  for(i in overlap.list)
  {
   ...
   second <- relatedOverlapBETTER(n.snps, i, 3, y2, N.tot = N_tot, 
    ...
    write.table(data.frame(round(cbind(first, second),digits = 6)) %>% set_colnames(c("A1", "B1", "C1","A2", "B2", "C2")),  
                file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/null_matrix_spearman/true_null_z/seed", seedi, ".replicate",rep,".", i, ".gwas.csv"),
