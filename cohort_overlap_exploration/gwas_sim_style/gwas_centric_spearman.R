...
for(rep in 1:niter)
{
  ...
  #Simulate new phenotypes
    ...
    {
      print(i)
      pop1.null[[i]] <- relatedOverlapBETTER(n.snps, i, 3, y1.null.similar, N.tot = 50000, 
                                             genotypes=pop1.geno, fixed.count = n.fixed)
      pop2.null[[i]] <- relatedOverlapBETTER(n.snps, i, 3, y2.null.similar, N.tot = 50000, 
                                             genotypes=pop2.geno, fixed.count = n.fixed)
      
      pop3.null[[i]] <- relatedOverlapBETTER(n.snps, i, 3, y2.null.similar, N.tot = 50000, 
    ...
    write.table(cor.def, file = paste0("/scratch16/abattle4/ashton/snp_networks/scratch/cohort_overlap_exploration/gwas_sim_style/iter",i,"res.csv"),quote = FALSE,sep = ",", row.names = FALSE )
